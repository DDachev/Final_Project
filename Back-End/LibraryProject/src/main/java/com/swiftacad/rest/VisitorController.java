...
public class VisitorController {
	...
	public ResponseEntity<Void> addVisitorInLibrary(@Valid @RequestBody Visitor visitor, @PathVariable("id") Long id) {
	...
	public ResponseEntity<Void> changeVisitor(@Valid @RequestBody Visitor visitor, @PathVariable("id") Long id) {
		Optional<Library> optional = libraryRepo.findById(id);
		if (optional.isPresent()) {
			visitor.setLibrary(optional.get());
			visitorRepo.save(visitor);
	...
	public ResponseEntity<Void> addBookToVisitorToReadIt(@PathVariable("v_id") Long v_id,
	...
	@RequestMapping(value = "visitors/{l_id}", method = RequestMethod.GET)
	public ResponseEntity<List<Visitor>> getAllVisitors(@PathVariable("l_id") Long l_id) {
		Optional<Library> optional = libraryRepo.findById(l_id);
		if (optional.isPresent()) {
			List<Visitor> visitors = new ArrayList<>();
			visitors = optional.get().getVisitors();
			if (visitors.isEmpty()) {
				return new ResponseEntity<>(HttpStatus.NOT_FOUND);
			}
			return new ResponseEntity<List<Visitor>>(visitors, HttpStatus.OK);
	...
	public ResponseEntity<Void> deleteVisitorById(@PathVariable("id") Long id) {
