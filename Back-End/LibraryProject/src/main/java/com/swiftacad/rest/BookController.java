...
public class BookController {
	...
	public ResponseEntity<Void> changeBook(@PathVariable("l_id") Long l_id, @Valid @RequestBody Book book) {
		Optional<Library> optional = libraryRepo.findById(l_id);
		if (optional.isPresent()) {
			book.setLibrary(optional.get());
			bookrepo.save(book);
	...
	@RequestMapping(value = "library/{l_id}/books", method = RequestMethod.GET)
	public ResponseEntity<List<Book>> getAllBooksFromLibrary(@PathVariable("l_id") Long l_id) {
		Optional<Library> optional = libraryRepo.findById(l_id);
		List<Book> books = new ArrayList<>();
		if (optional.isPresent()) {
			books = optional.get().getBooks();
